...
Original file is located at
    https://colab.research.google.com/drive/1bh4owLmShu43kTmQXL66KK3lmdGC8S7G
"""
...
def train(epoch):
    ...
    for i, data in enumerate(trainset):
        ...
        optimizer.step()
        if i % 100 == 0:
            print("Epoch:", epoch, "Step:", i * len(data), "/", len(trainset.dataset), "Loss:", loss.item() / len(data))
...
def importance_sampling(model, data, samples):
    ...
    for i,(x,z,m,s) in enumerate(zip(data,samples,mu,sig)): # iterate through a batch
        ...
        log_p_x_given_z = torch.sum((x * (x_hat).log() + (1 - x) * (1 - x_hat).log()).view(
            K, -1), 1).float()
...
def importance_sampling_eval(dataloader, K):
    ...
    for inputs in dataloader:
        ...
        samples = torch.Tensor([gen_samples(K, mi, si) for (mi,si) in zip(mu.data.numpy(),sig.data.numpy())])
...
samples = torch.Tensor([gen_samples(200, mi, si) for (mi,si) in zip(mu.data.numpy(),sig.data.numpy())])  #TODO
